...
class DQNAgent():
    ...
    def get_action(self, state):
        ...
        with torch.no_grad():
            input_state = torch.FloatTensor(state).unsqueeze(0).to(self.device)
            Q = self.model(input_state)
    ...
    def model_load(self, episode, comment="", eval=True):
        ...
        with open(load_path, "rb") as f:
            ...
            self.model.load_state_dict(torch.load(buffer, map_location="cuda:0"))
